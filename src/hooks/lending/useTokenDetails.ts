...
export const useTokenDetails = () => {
  ...
  const changePeriodDisplay = async ({
    tokenId,
    borrowPeriod,
    supplyPeriod,
  }) => {
    try {
      const isBorrowPeriodChange =
        borrowPeriod === 0 ||
        (borrowPeriod && borrowPeriod !== tokenBorrowDays?.length);
      const isSupplyPeriodChange =
        supplyPeriod === 0 ||
        (supplyPeriod && supplyPeriod !== tokenSupplyDays?.length);
      ...
      if (
        ...
        const { all } = await fetchTokenDetails(
          tokenId,
          borrowPeriod ?? supplyPeriod,
          true
        );
        docs = all;
      }
      ...
      if (isBorrowPeriodChange) {
        const displayDetails =
          borrowPeriod === 0
            ? docs
            : docs.slice(docs.length - borrowPeriod, docs.length);
      ...
      if (isSupplyPeriodChange) {
        const displayDetails =
          supplyPeriod === 0
            ? docs
            : docs.slice(docs.length - supplyPeriod, docs.length);
  ...
  const fetchTokenDetails = async (tokenId, period, isFetchOnly?: boolean) => {
    ...
    try {
      const [tokenDetailsRes, interestRateRes, currentUtilizationRes] =
        await Promise.allSettled([
          get_token_detail(tokenId, period),
          get_interest_rate(tokenId),
      ...
      const result = tokenDetails?.map((d) => {
        const supplyApyWithNet =
          Number(d.token_supply_apr) + Number(d.net_liquidity_apr || 0);
        d.tokenSupplyApy = supplyApyWithNet * 100;
        d.tokenBorrowApy = d.token_borrow_apr * 100;
        d.dayDate = new Date(d.createdAt).toLocaleDateString("en-GB", {
          day: "2-digit",
          month: "short",
          year: "numeric",
        });
      ...
      const interestRatesCal = interestRate?.utilization?.map((n, i) => {
        ...
        return {
          currentUtilRate:
            currentUtilization || lastTokenDetails.token_utilization_rate * 100,
          percent,
  ...
  const getCurrentUtilization = async (tokenId) => {
    ...
    if (token) {
      ...
      const totalSupplyD = new Decimal(supplied.balance)
        .plus(new Decimal(reserved))
        .toFixed();
      const currentUtilization =
        (borrowed.balance / Number(totalSupplyD)) * 100;
